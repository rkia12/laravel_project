...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12 mt-3 bg-white shadow rounded p-5 table-responsive">
            <h3 class="mb-3 fs-4">Modifier une ecole <a href="{{route("ecoles")}}" class="btn btn-secondary float-end">Retour</a></h3>
            <form action="{{ route('ecole.update',$ecole->id) }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="" for="nom">Nom de l'ecole :</label>
                    <input type="text" class="form-control @error('nom') is-invalid @enderror" name="nom" 
                        placeholder="Nom de l'ecole" value="{{ $ecole->nom }}"/>
                    @error('nom')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control  @error('id_ville') is-invalid @enderror"  name="id_ville" value="{{ old('id_ville') }}">
                        @if( !$villes->isEmpty())
                            <option value={{ $ecole->belongsToVille->id }} selected>{{ $ecole->belongsToVille->nomVille }}</option>
                            @foreach($villes as $ville)
                                @if($ecole->belongsToVille->id != $ecole->id_ville)
