...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12 mt-3 bg-white shadow rounded p-5 table-responsive">
            <h3 class="mb-3 fs-4">Ajouter un administrateur <a href="{{route("administrateurs")}}" class="btn btn-secondary float-end">Retour</a></h3>
            <form action="{{ route('administrateur.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="" for="password">Mote de passe:</label>
                    <input type="password" class="form-control @error('password') is-invalid @enderror" name="password" 
                        placeholder="Mote de passe" value="{{ old('password') }}"/>
                        @error('password')
                            <div class="invalid-feedback text-center">
                                <strong>{{ $message }}</strong>
                            </div>
